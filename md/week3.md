...
대부분 콘텐츠의 내용이 되는 글자 부분도 전부 이미지에 통으로 들어 있었고,<br>
버튼마저 이미지를 사용해서 활성화되면 색깔이 있는 이미지로 갈아끼우는 방식을 쓴 것 같았다.<br>
따라서 스와이프 UI를 키보드로도 조작 가능한 캐러셀 UI로 개선하고자 했으며<br>
스크린 리더를 쓰는 유저들도 내용을 충분히 얻을 수 있도록 개선점을 잡았다.
...
```
카드 안에는 기존에 background image만 있던 마크업에서 `<h2>`, `<p>`를 `sr-only`클래스로 추가해 카드를 포커싱하면 내용을 들을 수 있게 개선하였다.
